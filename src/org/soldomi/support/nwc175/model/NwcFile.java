...
public class NwcFile implements NwcItem {
    ...
    public List<NwcStaff> getStaves() {
	return m_staves;
    }
	...
	throws NwcFileException {
	reader.log("NwcFile : unmarshall started");
	...
	for (int i = 0; i < 12; i++) {
	    addFont(new NwcFont().unmarshall(reader));
	}

	reader.skip(4);
	int staffCount = reader.readByte();
	reader.skip(1);

	for (int i = 0; i < staffCount; i++) {
	    addStaff(new NwcStaff().unmarshall(reader));
	}

	reader.log("NwcFile : unmarshall finished");
